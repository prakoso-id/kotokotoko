    ...
    require([
            ...
            "dojo/domReady!"
              ...
              on
                ...
                var kelurahanLyrPoly = new FeatureLayer({
                    url: "https://sumurpantau.tangerangkota.go.id/arcgis/rest/services/Batas_kelurahan_area/MapServer/0",
                    id : "kelurahanLyr",
                    opacity: .00,
                    outFields: ["*"]
                });
                ...
                map = new Map({
                    basemap: "osm",
                    layers: [TngImageryLyr, kelurahanLyrPoly]
                });
                ...
                view.on("click", function(evt){
                    ...
                    const opts = {
                        include: kelurahanLyrPoly
                    }

                    view.hitTest(evt, opts).then(function(response) {
                        if (response.results.length) {
                            const graphic = response.results[0].graphic;
                            window.kodeWilayah = graphic.attributes.kdkel.toString();
                            selectKecamatan();
                        }
                    });
                });
        ...
        function selectKecamatan() {
            var id_kec = kodeWilayah.substr(4,2);
            $('[name=id_kec]').val(parseInt(id_kec)).trigger('change');
        }
</script>