<script type="text/javascript">
    ...
    function ubah_data(id){
        save_method = 'edit';
        $.ajax({
            url : "<?php echo base_url('berita/ajax_data')?>",
            type: "POST",
            data : {
                id : id,
                type : 'detail_berita',
            ...
            success: function(data){
                ...
                $('.form-control').removeClass('is-invalid');
                $('.help-block').empty();
                CKEDITOR.instances['pesan_keterangan'].setData(data.berita);
                $('[name="file_berita"]').val(data.foto);
                $('[name="id"]').val(data.id_berita);
                $('[name="judul"]').val(data.judul);
                $('.foto_berita').attr('src','<?php echo base_url('assets/images/berita/') ?>'+data.foto);
                $('#modal_tambah').modal('show');
        ...
        Swal.fire({
          ...
          cancelButtonText: 'Tidak',
            ...
            if (result.value) {
                ...
                if(save_method == 'add'){
                    var url = '<?php echo base_url("berita/ajax_save"); ?>';
                }else{
                    var url = '<?php echo base_url("berita/ajax_ubah"); ?>';
                }
    ...
    function hapus_data(id){
        Swal.fire({
          ...
          cancelButtonText: 'Tidak',
            ...
            if (result.value) {
                $.ajax({
                    url : '<?php echo site_url("berita/ajax_hapus"); ?>',
                    type: 'post',
                    data: {
                        id: id,
                        <?php echo $this->security->get_csrf_token_name(); ?> : '<?php echo $this->security->get_csrf_hash(); ?>'
    ...
    function restore_data(id){
        Swal.fire({
          ...
          cancelButtonText: 'Tidak',
            ...
            if (result.value) {
                $.ajax({
                    url : '<?php echo site_url("berita/ajax_restore"); ?>',
                    type: 'post',
                    data: {
                        id: id,
                        <?php echo $this->security->get_csrf_token_name(); ?> : '<?php echo $this->security->get_csrf_hash(); ?>'
...
document.querySelector('#upload-button').addEventListener('click', function() {
    ...
    request.open('post', '<?php echo base_url('berita/ajax_upload') ?>'); 
    request.send(data); 
});
</script>