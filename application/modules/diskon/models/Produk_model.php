...
class Produk_model extends CI_Model{
	...
	function get_produk_rekomendasi($id_jenis_usaha,$id_produk){
		$this->db->select('a.kode_produk,a.nama_produk,a.harga,a.id_produk,a.diskon,a.ratting,b.nama_usaha,(select foto from m_produk_foto  WHERE id_produk = a.id_produk LIMIT 1) as foto,(select COUNT(id_ulasan) from m_ulasan where id_produk = a.id_produk) as jumlah_ulasan,d.id_umkm as username,d.namausaha,e.nama_kec,e.nama_kel,f.id_wishlist');
		$this->db->from('m_produk a');
		$this->db->join('m_jenis_usaha b','b.id_jenis_usaha = a.id_jenis_usaha','left');
		$this->db->join('m_umkm d','d.id_umkm = a.id_umkm');
		$this->db->join('m_umkm_alamat e','e.id_umkm = d.id_umkm','left');
	...
	function get_balasan_diskusi($id_diskusi){
		$this->db->select('b.nama,b.username,a.*');
	...
	function get_produk_populer(){
		$this->db->select('a.kode_produk,a.nama_produk,a.harga,a.id_produk,a.diskon,a.ratting,b.nama_usaha,(select foto from m_produk_foto  WHERE id_produk = a.id_produk LIMIT 1) as foto,(select COUNT(id_ulasan) from m_ulasan where id_produk = a.id_produk) as jumlah_ulasan,d.id_umkm as username,d.namausaha,e.nama_kec,e.nama_kel,f.id_wishlist,(SELECT COUNT(id_wishlist) FROM m_wishlist WHERE id_produk = a.id_produk) as jum_wishlist');
		$this->db->from('m_produk a');
		$this->db->join('m_jenis_usaha b','b.id_jenis_usaha = a.id_jenis_usaha','left');
		$this->db->join('m_umkm d','d.id_umkm = a.id_umkm');
		$this->db->join('m_umkm_alamat e','e.id_umkm = d.id_umkm','left');
