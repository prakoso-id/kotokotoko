	...
	public function ajax_data(){
		...
		switch ($type) {
            ...
            case 'cari_nip':
                ...
                $useragent = 'Mozilla/5.0 (Windows NT 5.1; rv:31.0) Gecko/20100101 Firefox/31.0';
                $json_url = 'http://opendatav2.tangerangkota.go.id/services/pegawai/pegawaibynip/nip/'.$_POST['nip'];
                $ch = curl_init( $json_url );
                curl_setopt($ch, CURLOPT_USERAGENT, $useragent );
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            ...
            case 'data_propinsi':
                $useragent = 'Mozilla/5.0 (Windows NT 5.1; rv:31.0) Gecko/20100101 Firefox/31.0';
                $json_url = 'http://opendatav2.tangerangkota.go.id/services/wilayah/propinsi_all';
                $ch = curl_init( $json_url );
                curl_setopt($ch, CURLOPT_USERAGENT, $useragent );
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                curl_setopt($ch, CURLOPT_USERPWD, REST_U . ':' . REST_P);
            ...
            case 'data_kota':
                ...
                {
                    $json_url = 'http://opendatav2.tangerangkota.go.id/services/wilayah/kabupaten/no_prop/'.$this->input->post('data_propinsi',true);
                }else{
                    $json_url = 'http://opendatav2.tangerangkota.go.id/services/wilayah/kabupaten/no_prop/36';
                }
                $ch = curl_init( $json_url );
                curl_setopt($ch, CURLOPT_USERAGENT, $useragent );
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            ...
            case 'data_kecamatan':
                if($this->input->post('data_propinsi'))
                {
                    $json_url = 'http://opendatav2.tangerangkota.go.id/services/wilayah/kecamatan/no_prop/'.$this->input->post('data_propinsi',true).'/no_kab/'.$this->input->post('data_kota',true);
                }else{
                    $json_url = 'http://opendatav2.tangerangkota.go.id/services/wilayah/kecamatan/no_prop/36/no_kab/71';
                }
                $useragent = 'Mozilla/5.0 (Windows NT 5.1; rv:31.0) Gecko/20100101 Firefox/31.0';
                $ch = curl_init( $json_url );
                curl_setopt($ch, CURLOPT_USERAGENT, $useragent );
            ...
            case 'data_kelurahan':
                ...
                if($this->input->post('data_propinsi'))
                {
                    $json_url = 'http://opendatav2.tangerangkota.go.id/services/wilayah/kelurahan/no_prop/'.$this->input->post('data_propinsi',true).'/no_kab/'.$this->input->post('data_kota',true).'/no_kec/'.$this->input->post('data_kec',true);                    
                }else{
                    $json_url = 'http://opendatav2.tangerangkota.go.id/services/wilayah/kelurahan/no_prop/36/no_kab/71/no_kec/'.$this->input->post('data_kec',true);
                }
                ...
                foreach ($arr_kat as $k) {
                    ...
                    foreach ($k as $val) {
                        ...
                        $html .= '<a href="javascript:void(0);" onclick="list_produk('.$val->id_jenis_usaha.',`'.$val->nama_usaha.'`)" class="list-group-item list-group-item-action">
                                    '.$val->nama_usaha.'
                                    <span class="badge badge-primary badge-pill jum_kategori_'.$val->id_jenis_usaha.'">'.$jum.'</span>
                                </a>';
            ...
            break;
			case 'data_ecommerce':
                $query['select']    = 'situs_web';
                $query['table']     = 'm_umkm';
                $query['where']     = 'id_umkm = "'.(int)$this->input->post('id_umkm',true).'" AND username = "'.$this->session->identity.'"';  
                $result = $this->query_model->getRow($query);

                $form_ecommerce = '';
                if (isJSON($result->situs_web)) {
                    $array_web = json_decode($result->situs_web);
                    foreach ($array_web as $row) {
                        $form_ecommerce .= '<div class="position-relative row form-group">
                                                ...
                                                <div class="col-lg-9">
                                                    <input type="text" name="link_produk[]" class="form-control" id="link_'.$row->nama_ecommerce.'" placeholder="Link Produk">
            ...
            case 'cek_status_verif_umkm':
                ...
                if ($result->id_status != 1) {
                    ...
                    $message = '<strong>Peringatan !</strong> <br> 
                                ...
                                Jika status verifikasi di tolak oleh admin maka produk yang sudah Anda tambahkan akan di nonaktifkan / unpublihs.';
                }
	...
	public function ajax_save(){
        ...
        switch ($type) {
            case 'data_pengguna':
                ...
                $useragent = 'Mozilla/5.0 (Windows NT 5.1; rv:31.0) Gecko/20100101 Firefox/31.0';
                $json_url = 'http://opendatav2.tangerangkota.go.id/services/unor/get_unor_by_kode/kode_unor/'.substr($this->input->post('kode_unor',true), 0, 5);;
                $ch = curl_init( $json_url );
                curl_setopt($ch, CURLOPT_USERAGENT, $useragent );
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                curl_setopt($ch, CURLOPT_USERPWD, REST_U . ':' . REST_P);
            ...
            switch ($type) {
                case 'transaksi':
                    ...
                    foreach ($list as $l) {
                        ...
                        $l->aksi = '<a href="javascript:void(0);" onclick="detail_transaksi('.$l->id_transaksi.')" title="Detail Transaksi" class="btn btn-success"><i class="fa fa-eye"></i></a>'; 
                        $data[] = $l;
                    }
