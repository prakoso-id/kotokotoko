...
class Template
{
	 ...
	 * @param string $content
	 */
	public function add_breadcrumb($links=[], $title=null, $subtitle=null)
	{
		//breadcrumb utk backend
		$bc = array();
		$bc[] = '<div class="content-header">';
		$bc[] = '<div class="container-fluid">';
		$bc[] = '<div class="row mb-2">';
		$bc[] = '<div class="col-sm-6">';
		if ($title)
			$bc[] = '<h1 class="m-0 text-dark">'.$title.($subtitle?' <small>'.$subtitle.'</small>':'').'</h1>';
		else {
			if (count($this->_title_segments) > 0) {
				$page_title = implode($this->_title_separator, array_reverse($this->_title_segments));
				$bc[] = '<h1 class="m-0 text-dark">'.$page_title.'</h1>';
			}
		}
		$bc[] = '</div>';
		$bc[] = '<div class="col-sm-6">';
		$bc[] = '<ol class="breadcrumb float-sm-right">';
		$lastElement = end($links);
		foreach ($links as $blink => $btitle) {
			$isactive = $blink == $lastElement;
			$bc[] = '<li class="breadcrumb-item'.($isactive ? ' active' : '').'">';
			if ($isactive)
				$bc[] = $btitle;
			else
				$bc[] = '<a href="'.$blink.'">'.$btitle.'</a>';
			$bc[] = '</li>';
		}
		$bc[] = '</ol>';
		$bc[] = '</div>';
	...
	public function render($view, $data = array(), $return = FALSE)
	{
		...
		$this->set('breadcrumb', $this->_breadcrumbs);
